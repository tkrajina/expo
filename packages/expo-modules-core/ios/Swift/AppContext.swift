...
public class AppContext {
  ...
  /**
   Handles client app (`UIApplication`) lifecycle notifications and posts appropriate events to the module registry.
   */
  @objc
  private func handleClientAppNotification(_ notification: Notification) {
    switch notification.name {
    case UIApplication.willEnterForegroundNotification:
      moduleRegistry.post(event: .clientAppEnterForeground)
    case UIApplication.didBecomeActiveNotification:
      moduleRegistry.post(event: .clientAppBecomeActive)
    case UIApplication.didEnterBackgroundNotification:
      moduleRegistry.post(event: .clientAppEnterBackground)
    default:
      return
    }
  }
  ...
  deinit {
    ...
    moduleRegistry.post(event: .appContextDestroy)
  }
}
