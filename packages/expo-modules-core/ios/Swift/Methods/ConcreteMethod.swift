...
public struct ConcreteMethod<Args, ReturnType>: AnyMethod {
  ...
  public func call(args: [Any?], promise: Promise) {
    call(args: args, promise: promise, type: ReturnType.self)
  }

  public func call<T>(args: [Any?], promise: Promise, type: T.Type) {
    do {
      let finalArgs = try castArguments(args) + (takesPromise ? [promise] : [])
      let tuple = try Conversions.toTuple(finalArgs) as! Args
      let returnedValue = call(tuple: tuple, type: T.self)
  ...
  public func call<T>(args: [Any?], promise: Promise, type: T.Type) where T: RawRepresentable {
    do {
      let finalArgs = try castArguments(args) + (takesPromise ? [promise] : [])
      let tuple = try Conversions.toTuple(finalArgs) as! Args
      let returnedValue = call(tuple: tuple, type: T.self)
  ...
  public func call<T>(tuple: Args, type: T.Type) -> String {
    return String(describing: closure(tuple))
  }

  public func call<T>(tuple: Args, type: T.Type) -> String where T: RawRepresentable {
    return String(describing: closure(tuple))
  }
